...
import { TTS_MIME_TYPE, openAiTextToSpeech } from "../voice/openai-speech";
import { VoiceUnavailableError, type TextToSpeech } from "../voice/speech";
import { reserveTutorSpeech, settleTutorSpeech } from "./money";
...
/**
 * The whole behaviour of `POST /api/tutor/speak`, with the vendor INJECTED — the
 * house pattern (`AudioModelClient`, `SpeakerEmbedder`): the caller chooses the
 * implementation, no module-level singleton decides, and the test drives it with a
 * plain fake that makes no network call.
 *
 * It lives here rather than in the route file because Next.js allows a route module
 * to export ONLY its HTTP verbs and a small set of config fields — an extra export is
 * a hard BUILD failure that `tsc --noEmit` does not see. (Which is the point of
 * running the real build: this exact mistake was caught by `npm run build` and by
 * nothing else.)
 */
