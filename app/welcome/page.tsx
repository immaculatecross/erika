...
import { RequirementsStep } from "@/components/onboarding/requirements-step";
import { SpeakStep } from "@/components/onboarding/speak-step";
...
export default function WelcomePage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/onboarding")
      .then((r) => r.json())
      .then((body: OnboardingView) => alive && setView(body))
      .catch(() => {});
    fetch("/api/placement")
      .then((r) => r.json())
  ...
  return (
    <Frame>
      {view ? (
        ...
        <p className="text-[15px] text-secondary">Loading…</p>
      )}
    </Frame>
  );
}
