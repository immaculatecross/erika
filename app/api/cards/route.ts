...
import { listCardBrowserViews, listDueCardViews } from "@/lib/cards";
...
// due first, each carrying its due date and suspended state. Faces are re-derived
// correction-forward from each card's finding at read time (E-29). GET only.
...
export async function GET(request: Request) {
  ...
  if (new URL(request.url).searchParams.get("due") === "1") {
    const cards = listDueCardViews(db);
    return NextResponse.json({ cards, dueCount: cards.length });
  }
  return NextResponse.json({ cards: listCardBrowserViews(db) });
}
