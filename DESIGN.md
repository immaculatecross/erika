...
- **Streak:** a number and a word ("Day 14"), caption style, sentence case — the uppercase caption token is for section labels, not for this line. The number is the run's **span**, so it includes any repaired day; the repair disclosure is therefore not decoration but the thing that keeps the number honest, and it is never dropped. Repairs are shown factually: **named while there are one or two** ("Day 14 · repaired Tue"), and **counted once there are more** ("Day 162 · 12 repaired"). The summarised form exists because repairs accrue at two a month with no cap on run length — enumerating a long run's repairs would render an itemised list of every day you missed, which is neither a number and a word nor caption length on a phone, and is the closest this surface could come to a guilt ledger. Summarise; never truncate — a count that omits repairs would let the number absorb days the learner did not practise. No flames, no guilt copy on a broken streak, and a run of zero renders nothing at all.
