...
| `renditions` | 12 | `finding_id` | One contrastive-playback rendition per finding (E-21): `path` (mp3 under `data/renditions/`), `cost_usd` (the actual TTS charge, also ledgered once). The `finding_id` PK is both the render-once cache key and the lease-before-spend concurrency guard (the row is claimed before the TTS call, so a double-clicked Generate makes exactly one provider call and one ledger row). FK cascade on delete; the file is cleaned by the delete route and playback is orphan-safe without it. |
