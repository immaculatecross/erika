...
| Cold-start walkthrough | **PASS** — on the fifth attempt (four killed by platform failures before producing a verdict). A no-context session went from clean worktree to **findings** with one real recording, following only `README.md` and the product's own screens. Onboarding's first screen states all four prerequisites v0.6 left a newcomer to discover by failing. The planted error (`ho andato` → `sono andato`) was caught with the correct explanation, and no Analyze button was pressed at any point. **First pass in this project's history** — v0.4 and v0.6 both failed it. |
