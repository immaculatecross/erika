...
describe("buildFocusPayload — buckets the included findings (data path)", () => {
  ...
  it("distributes a session's findings by its LOCAL capture hour", () => {
    ...
    // [E-39 §B2] The CAPTURE time is the basis of the histogram. `created_at` is
    // deliberately left as the upload instant ("now") and must not influence a bucket.
    ...
    expect(payload.slipHours.unknownTime).toBe(0);
  ...
  it("counts a finding with NO capture time as unknown rather than filing it by upload hour", () => {
    // [E-39 §B2] The defect this replaces: a recording with no capture metadata was
    // bucketed by `created_at`, so an evening uploader's whole histogram was one bar at
    // the hour they upload, under the heading "your local time". The honest answer is to
    // leave it out and say how many were left out.
    const db = freshDb();
    createSession(db, { id: "s2", originalFilename: "s.wav", format: "wav", sizeBytes: 1, durationSeconds: 3600 });
    // captured_at stays NULL — a picked file with no container tag, the common case.
    upsertSegment(db, { sessionId: "s2", idx: 0, startMs: 0, endMs: 3_600_000, contentHash: "s2-h" });
    persistSegmentFindings(db, {
      sessionId: "s2",
      contentHash: "s2-h",
      flagged: true,
    ...
    const job = enqueueAnalysis(db, "s2");
    db.prepare("UPDATE analysis_jobs SET state='done', progress=1 WHERE id=?").run(job.id);

    const payload = buildFocusPayload(db);
    expect(payload.slipHours.unknownTime).toBe(1);
    expect(payload.slipHours.total).toBe(0);
    expect(payload.slipHours.buckets.every((n) => n === 0)).toBe(true);
    expect(payload.slipHours.peakHour).toBeNull();
  });
});
